...
async function tryExecuteTx(deployer, txnParams) {
  try {
    ...
    console.error(
      "Transaction Failed",
      e.response?.body ?? e.response ?? e.body ?? e
    );
  }
}
...
async function getOpenWagers(deployer, wagerAppID) {
  ...
  for (const acc of accounts.accounts) {
    ...
    const wagerState = Buffer.from(
      localState.get("wager_state") ?? "",
      "base64"
    ).toString();
    ...
    if (wagerState === "PENDING" && now < limitDate) {
      openWagers[acc.address] = {
        ...
        wager_bet_direction: Buffer.from(
          localState.get("wager_bet_direction") ?? "",
          "base64"
        ).toString(),
      };
    }
...
async function getLiveWagers(deployer, wagerAppID) {
  ...
  for (const acc of accounts.accounts) {
    ...
    if (
      ...
      localState.get("asa_price_at_wager_accept") !== undefined
    ) {
      continue;
    }
    ...
    liveWagers.push([
      ...
      },
      {
        ...
        wager_state: Buffer.from(
          againstAccLocalState.get("wager_state") ?? "",
          "base64"
        ).toString(),
        wager_amount: againstAccLocalState.get("wager_amount"),
        wager_bet_direction: Buffer.from(
          againstAccLocalState.get("wager_bet_direction") ?? "",
          "base64"
        ).toString(),
        asa_price_at_wager_accept: againstAccLocalState.get(
          "asa_price_at_wager_accept"
        ),
      },
    ]);
